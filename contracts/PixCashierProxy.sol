    ...
    IPixCashierErrors
{
    ...
    function cashInBatch(
        ...
        for (uint256 i = 0; i < account.length; i++) {
            Error err = _shard(txId[i]).registerCashIn(account[i], amount[i], txId[i], CashInStatus.Executed);
            if (err != Error.None) {
                if (err == Error.ZeroAccount) revert ZeroAccount();
                if (err == Error.ZeroAmount) revert ZeroAmount();
                if (err == Error.ZeroTxId) revert ZeroTxId();
            ...
            emit CashIn(account[i], amount[i], txId[i]);

            if (!IERC20Mintable(_token).mint(account[i], amount[i])) {
                revert TokenMintingFailure();
            }
        }
